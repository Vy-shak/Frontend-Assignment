...
function page() {
  return (
    <section className='w-full pt-12   flex flex-col justify-start items-center  h-full'>
        <Sidebar/>
        <div className='w-full h-16 bg-white border-b-2 p-3 flexSide'>
        ...
        <div className='w-full flex-col gap-y-3  l px-4 md:pr-10 md:pl-16 lg:pr-14 lg:pl-56 xl:pr-20 xl:pl-64 h-16 bg-white border-b-2 p-3 flexStart'>
            {mentorsList.map((item,index)=>(
                <Chatbox key={index} verified={item.verified} role={item.role} Name={item.Name} Profile={item.Profile} notifications={item.notifications} />
            ))}
        </div>
    </section>
  )
